...
//        onCreate(db);
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        sm.put(COL_DIST, 123.25);
        sm.put(COL_NAME, "yuriyuripps");
        db.insert(SQL_CREATE_TABLE_ARM, null, sm);

        sm = new ContentValues();
        sm.put(COL_DIST, 124.25);
        sm.put(COL_NAME, "yuriyuripps");
        db.insert(SQL_CREATE_TABLE_ARM, null, sm);

        sm = new ContentValues();
        sm.put(COL_DIST, 128.21);
        sm.put(COL_NAME, "yuriyuripps");
        db.insert(SQL_CREATE_TABLE_ARM, null, sm);

        sm = new ContentValues();
        sm.put(COL_DIST, 132.24);
        sm.put(COL_NAME, "yuriyuripps");
        db.insert(SQL_CREATE_TABLE_ARM, null, sm);

        sm = new ContentValues();
        sm.put(COL_DIST, 129.54);
        sm.put(COL_NAME, "yuriyuripps");
        db.insert(SQL_CREATE_TABLE_ARM, null,sm);
    ...
    public void updateDistSm(float dist , String name){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(COL_DIST, dist);
